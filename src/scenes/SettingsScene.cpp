        ...
        if (m_rebindStep >= 0) {
            if (const auto* k = event->getIf<sf::Event::KeyPressed>()) {
                ...
                if (sc == sf::Keyboard::Scancode::Escape) {
                    ...
                    m_rebindStep++;
                    if (m_rebindStep >= static_cast<int>(m_rebindActions.size())) m_rebindStep = -1;
...
void SettingsScene::selectPreviousItem() {
    m_selectedIndex--;
    if (m_selectedIndex < 0) m_selectedIndex = static_cast<int>(m_itemTexts.size()) - 1;
    refreshItemTexts();
}

void SettingsScene::selectNextItem() {
    m_selectedIndex++;
    if (m_selectedIndex >= static_cast<int>(m_itemTexts.size())) m_selectedIndex = 0;
    refreshItemTexts();
}

void SettingsScene::adjustCurrentVolume(float delta) {
    if (m_selectedIndex < 0 || m_selectedIndex >= static_cast<int>(m_itemActions.size())) return;
    SettingsItem item = m_itemActions[m_selectedIndex];
    if (item == SettingsItem::Back || item == SettingsItem::RebindKeys) return;

    ConfigManager& cfg = Game::getInstance().getConfigManager();
    float current = 0.0f;
    switch (item) {
    case SettingsItem::MasterVolume: current = cfg.getMasterVolume(); break;
    case SettingsItem::BGMVolume:    current = cfg.getBGMVolume();    break;
    case SettingsItem::SFXVolume:    current = cfg.getSFXVolume();    break;
    default: return;
    }
...
void SettingsScene::handleRebindKey(sf::Keyboard::Scancode scancode) {
    if (m_rebindStep < 0 || m_rebindStep >= static_cast<int>(m_rebindActions.size())) return;

    auto [player, action] = m_rebindActions[m_rebindStep];
    std::string keyName = InputManager::scancodeToString(scancode);
    if (keyName == "Unknown") return;  // 未知/不支持的键忽略
